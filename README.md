...
│
├── generator-union.py        # 结果加权脚本：对网络进行加权
│
...
`Data/` 仅保留上述 4 个代表性网络，完整实验数据（如 Pokec 百万节点网络）可自行下载。
...
# DomiRank-Iter⁺：加速迭代求解器（对迭代解的加速）
python domirank-main/main_iteration+.py

# DomiRank-Anal⁺：解析求解器（解析解的加速方法）
python domirank-main/main_ana+.py

# 对应的网络加权（基于社区脆弱性，邻域脆弱性）
...
- **DomiRank-Iter⁺**：通过结构等价节点压缩、惰性更新与 Anderson 加速，在大规模图上实现 1–2 个数量级的效率提升，注意在计算的时候要注意不同数据集可能需要不同的参数。
